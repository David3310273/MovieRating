...
if __name__ == '__main__':
    os.environ["PYSPARK_PYTHON"] = "/Users/david/.pyenv/versions/3.5.2/bin/python"
    os.environ["PYSPARK_DRIVER_PYTHON"] = "/Users/david/.pyenv/versions/3.5.2/bin/python"
    ...
    recommend_list = movie_common.join(userlist).map(lambda line: (line[1][0], line[1][1])).reduceByKey(lambda a, b: [a]+[b] if type(a) != list and type(b) != list else (a+b if type(a) == list and type(b) == list else (a+[b] if type(a) == list else [a]+b))).map(lambda line: (line[0], set(line[1])))
    # result stores in redis

    for item in recommend_list.toLocalIterator():
        conn.hset("recommends", item[0][0], item[0][1])
